...
# DBTITLE 1,Create Allele ID Mapping Reference Table

from pyspark.sql.types import StructType, StructField, StringType

print("CREATING ALLELE ID MAPPING REFERENCE TABLE")
print("=" * 50)

# Explicit schema to avoid numeric overflow / auto-casting
allele_schema = StructType([
    StructField("variation_id", StringType(), True),
    StructField("allele_id", StringType(), True)
])

# Path in DBFS / UC volume (update if needed)
allele_csv_path = "/path/to/allele_id_mapping.csv"

print(f"Reading allele mapping from: {allele_csv_path}")
...
df_allele_mapping = (
    spark.read
        ...
        .load(allele_csv_path)
)

print("Writing table: workspace.reference.allele_id_mapping")
...
(
    df_allele_mapping
        ...
        .saveAsTable("workspace.reference.allele_id_mapping")
)

print("SUCCESS: Allele ID mapping table created")

print("Schema verification:")
df_allele_mapping.printSchema()

print("Sample rows:")
df_allele_mapping.show(5, truncate=False)