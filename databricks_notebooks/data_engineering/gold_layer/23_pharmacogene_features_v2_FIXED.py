...
# MAGIC
# MAGIC TARGET DEFINITION (GOLD V2):
# MAGIC   has_strong_pharmacogene_evidence = True when:
# MAGIC     (is_validated_cancer_target AND druggability_score >= 0.7) OR
# MAGIC     (drug_relationships >= 10 AND pathogenic_variants >= 3)
# MAGIC
# MAGIC   Rationale: Old target is_high_priority_pharmacogene used clinical_utility_score
# MAGIC   which included has_pharmgkb_annotation. Since ALL rows in this table come from
# MAGIC   PharmGKB, 95.6% were marked "high priority" (inverted target).
# MAGIC   New target requires either: (1) Validated target + high druggability OR
# MAGIC   (2) Strong drug evidence (10+ relationships) + multiple pathogenic variants.
# MAGIC   Expected positive rate: 20-30%.
# MAGIC
# MAGIC   GOLD V2 FIX: March 13, 2026 - Tier 1 broken target fix

# COMMAND ----------

# DBTITLE 1,PASS 2 - Derive Target Variable
print("\nPASS 2 - DERIVING TARGET VARIABLE (GOLD V2)")
print("="*80)
print("Target: has_strong_pharmacogene_evidence")
print("Definition: (validated_target + high_druggability) OR (10+ drug_rels + 3+ variants)")
print()
...
df_target = (
    ...
    .withColumn("has_strong_pharmacogene_evidence",
                when(
                    # Option 1: Validated target + high druggability
                    (col("is_validated_cancer_target") == True) &
                    (coalesce(col("druggability_score"), lit(0.0)) >= 0.7),
                    True
                ).when(
                    # Option 2: Strong drug evidence + multiple variants
                    (coalesce(col("drug_relationships"), lit(0)) >= 10) &
                    (coalesce(col("pathogenic_variants"), lit(0)) >= 3),
                    True
                ).otherwise(False))
    .select("gene_symbol", "has_strong_pharmacogene_evidence")
)

target_counts = df_target.groupBy("has_strong_pharmacogene_evidence").count().collect()
total = sum(r["count"] for r in target_counts)
for row in sorted(target_counts, key=lambda r: str(r["has_strong_pharmacogene_evidence"])):
    pct = row["count"] / total * 100
    print(f"  {row['has_strong_pharmacogene_evidence']}: {row['count']:,} ({pct:.2f}%)")

positives = [r["count"] for r in target_counts if r["has_strong_pharmacogene_evidence"] == True]
...
if positive_count == 0:
    ...
    raise ValueError(f"Positive rate {positive_pct:.2f}% too low (< 5%). Fix threshold.")
elif positive_pct > 80.0:
    raise ValueError(f"Positive rate {positive_pct:.2f}% too high (> 80%). Fix threshold.")
elif positive_pct < 15.0 or positive_pct > 35.0:
    print(f"WARNING: Positive rate {positive_pct:.2f}% outside target range (20-30%). Review if intentional.")
else:
    print(f"OK: Positive rate {positive_pct:.2f}% within target range (20-30%). Proceeding.")
...
df_final = (
    ...
    .fillna({"has_strong_pharmacogene_evidence": False})
)
...
target_dist = df_check.groupBy("has_strong_pharmacogene_evidence").count().collect()
total       = sum(r["count"] for r in target_dist)
positives   = [r["count"] for r in target_dist if r["has_strong_pharmacogene_evidence"] == True]
...
# MAGIC %md
# MAGIC ### GOLD V2 VALIDATION
# MAGIC Comprehensive validation queries to verify target quality and check for leakage.
# MAGIC Run these queries manually in SQL notebook after table write completes.

# COMMAND ----------

# DBTITLE 1,GOLD V2 VALIDATION - Target Distribution
# MAGIC %sql
# MAGIC -- UC10: Target Distribution Check
# MAGIC -- Expected: 20-30% positive (was 95.6% in V1)
# MAGIC
# MAGIC SELECT 
# MAGIC   has_strong_pharmacogene_evidence,
# MAGIC   COUNT(*) as row_count,
# MAGIC   ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER(), 2) as percent
# MAGIC FROM workspace.gold.gene_pharmacogene_ml_features
# MAGIC GROUP BY has_strong_pharmacogene_evidence
# MAGIC ORDER BY has_strong_pharmacogene_evidence;
# MAGIC
# MAGIC -- PASS if: TRUE = 20-30%, FALSE = 70-80%
# MAGIC -- FAIL if: TRUE > 80% or TRUE < 5%

# COMMAND ----------

# DBTITLE 1,GOLD V2 VALIDATION - Sample Positive Rows
# MAGIC %sql
# MAGIC -- Sample positive rows to verify logic
# MAGIC SELECT 
# MAGIC   gene_symbol,
# MAGIC   is_validated_cancer_target,
# MAGIC   druggability_score,
# MAGIC   drug_relationships,
# MAGIC   pathogenic_variants,
# MAGIC   has_strong_pharmacogene_evidence
# MAGIC FROM workspace.gold.gene_pharmacogene_ml_features
# MAGIC WHERE has_strong_pharmacogene_evidence = TRUE
# MAGIC LIMIT 20;
# MAGIC
# MAGIC -- Verify: Either validated_target=TRUE + druggability>=0.7
# MAGIC --     OR: drug_relationships>=10 + pathogenic_variants>=3

# COMMAND ----------

# DBTITLE 1,GOLD V2 VALIDATION - Correlation Leakage Scan
# MAGIC %sql
# MAGIC -- Quick correlation scan for leakage
# MAGIC -- Flag any correlation >= 0.98
# MAGIC
# MAGIC SELECT 
# MAGIC   ROUND(corr(CAST(has_strong_pharmacogene_evidence AS INT), clinical_annotation_count), 4) as corr_clinical_annot,
# MAGIC   ROUND(corr(CAST(has_strong_pharmacogene_evidence AS INT), pharmacogene_variant_count), 4) as corr_variant_count,
# MAGIC   ROUND(corr(CAST(has_strong_pharmacogene_evidence AS INT), cancer_mutation_count), 4) as corr_cancer_mutations,
# MAGIC   ROUND(corr(CAST(has_strong_pharmacogene_evidence AS INT), drug_relationships), 4) as corr_drug_rels,
# MAGIC   ROUND(corr(CAST(has_strong_pharmacogene_evidence AS INT), druggability_score), 4) as corr_druggability
# MAGIC FROM workspace.gold.gene_pharmacogene_ml_features;
# MAGIC
# MAGIC -- PASS if: All correlations < 0.98
# MAGIC -- WARN if: Any correlation 0.85-0.98 (review feature)
# MAGIC -- FAIL if: Any correlation >= 0.98 (leakage)

# COMMAND ----------

# DBTITLE 1,GOLD V2 VALIDATION - Row Count Stability
# MAGIC %sql
# MAGIC -- Verify row count stable vs V1 (~2,209 rows expected)
# MAGIC SELECT COUNT(*) as total_rows
# MAGIC FROM workspace.gold.gene_pharmacogene_ml_features;
# MAGIC
# MAGIC -- PASS if: 2,100 - 2,300 rows (within 5% of V1)
# MAGIC -- FAIL if: < 2,000 or > 2,500 (major data loss or duplication)

# COMMAND ----------

print("\n" + "="*80)
print("VALIDATION QUERIES DEFINED")
print("="*80)
print("Run the SQL cells above manually to validate:")
print("  1. Target distribution: 20-30% positive")
print("  2. Sample rows verify logic")
print("  3. No correlation >= 0.98 (leakage)")
print("  4. Row count stable (~2,209)")
print()
print("If all checks PASS → Proceed to Tier 2+3 fixes for remaining tables")
print("If any check FAILS → Adjust target threshold and re-run")
print("="*80)
